class Inventory::InventoryController < Inventory::ApplicationController
	...
	def show
		...
		if @item.type == "ContactsInventory" then
			redirect_to inventory_contact_path(@item)
		else
			if @item.type == "FramesInventory" then
				redirect_to inventory_frame_path(@item)
			else
				if @item.type == "LensInventory" then
					redirect_to inventory_lens_path(@item)
				else
					if @item.type == "ServiceInventory" then
						redirect_to inventory_service_path(@item)
					else
						redirect_to inventory_other_path(@item)
					end
				end
			end
		end
